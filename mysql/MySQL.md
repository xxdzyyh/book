...
* 共享锁：表示对数据进行读操作，因此多个事务可以同时为一个对象加共享锁

* 排他锁：表示对数据进行写操作，如果一个事务对对象加了排它锁，其他事务就不能再给它在添加任何锁了。

* 行锁：对一行记录加锁，InnoDB行锁是通过给索引上的索引项加锁来实现的，这一点MySQL与Oracle不同，后者是通过在数据块中对相应数据行加锁来实现的。

  InnoDB这种行锁实现特点意味着：只有通过索引条件检索数据，InnoDB才使用行级锁，否则，InnoDB将使用表锁

* 意向锁：意向锁是表级锁，其设计目的主要是为了在一个事务中揭示下一行将要被请求锁的类型。InnoDB 中的两个表锁：
...
* 行级锁：对行进行加锁，开销大，会出现死锁，加锁慢，锁粒度最小，发生冲突概率最低，并发高
